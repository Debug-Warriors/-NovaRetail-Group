...
with col2:
    ...
    risk = st.session_state.graph_state.get(
...
with col3:
    ...
    score = st.session_state.graph_state.get(
...
with col4:
    ...
    confidence = st.session_state.graph_state.get(
...
# Use suggested prompt from sidebar if available
default_prompt = ""
...
approval_required = (
    ...
    graph_state.get("current_agent")
    == "Risk Intelligence Agent"

    and

    graph_state.get("overall_risk")
    in ["High", "Critical"]

    and

    not st.session_state.approval
...
if approval_required:
    ...
    st.warning(
        "⚠️ High-risk disruption detected.\n"
        "Approval is required before "
        "preventive actions are executed."
    )
